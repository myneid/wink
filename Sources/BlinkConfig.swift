...
final class BlinkConfig {
  ...
  private func writeSSHConfig(keys: [ArchivedKey]) throws {
    ...
    // The file `ssh -F` is pointed at for Hosts menu connections: Blink's hosts
    // first (first value wins in ssh), then your own ~/.ssh/config.
    let launch = """
      # Generated by Wink. Used by the Hosts menu.
      Include \(Self.hostsConfURL.path)

      Match all
        Include ~/.ssh/config

      """
    try launch.write(to: Self.launchConfURL, atomically: true, encoding: .utf8)
  }
  ...
  func sshCommand(for host: BlinkHost) -> [String] {
    ["ssh", "-F", Self.launchConfURL.path, host.sshAlias]
  }
